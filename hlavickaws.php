...
class HlavickaWs extends WebovaSluzbaNG {
	...
	public function Odpoved() {
		if (isset($_GET["backup"])){
			...
			$odpoved = $this->NacitajHlavicku(true);
	...
	public function NacitajHlavicku($backup=false) {
		if ($backup) {
			$vsetkyKombinacie = $this->vratVsetkyKombinacie();
			foreach ($vsetkyKombinacie as $id_pouzivatela => $portfolia) {
				foreach ($portfolia as $portfolio) {
					$this->id_pouzivatela=$id_pouzivatela;
					$this->id_portfolia=$portfolio;
...
WHERE RU.ID_POUZIVATELA=$this->id_pouzivatela AND MP.ID_PORTFOLIA $port";
	...
	private function vratVsetkyKombinacie(){
		$polePouzivatelov = array();
		$polePortfolii =array();
		$sql = "Select ID_POUZIVATELA from REGISTROVANY_POUZIVATEL";
		$kurzor = $this->data->Query($sql);
		while ($riadok = $this->data->StiahniRiadok($kurzor)) {
			array_push($polePouzivatelov, $riadok["ID_POUZIVATELA"]);
		}
		foreach ($polePouzivatelov as $pouzivatel) {
			$polePortfolii[$pouzivatel] = array(0);
		}
		$sql="SELECT ID_POUZIVATELA, ID_PORTFOLIA from portfolio";

		$kurzor = $this->data->Query($sql);
		while ($riadok = $this->data->StiahniRiadok($kurzor)) {
			array_push($polePortfolii[$riadok["ID_POUZIVATELA"]], $riadok["ID_PORTFOLIA"]);
		}

		return $polePortfolii;
	}
